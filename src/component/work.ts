...
import { Planet, Contract } from "../planet";
...
Vue.component('work', {
  ...
  computed: {
    ...
    percent():       number  { return Math.min(100, Math.ceil(this.turnsWorked / this.turns * 100)) },
    timeSpent():     number  { return Math.floor(this.turnsWorked / data.turns_per_day) },
    hasPicketLine(): boolean { return this.planet.hasPicketLine() },
    turns():         number  { return this.days * (24 / data.hours_per_turn) },
    pay():           number  { return (this.task) ? this.payRate * this.days : 0 },
    payRate():       number  { return this.task ? this.getPayRate(this.task) : 0 },

    tasks() {
      const planet: Planet = this.planet;
      return planet.work_tasks.map(name => data.work.find(work => work.name == name));
    },
  ...
  methods: {
    ...
    performWork() {
      if (this.task && this.isReady && !this.isFinished) {
        ...
        let timer: number; timer = window.setInterval(() => {
          ++this.turnsWorked;
